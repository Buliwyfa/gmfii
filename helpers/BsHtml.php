...
* BsHtml class file.
* @author b3457m0d3
* @copyright Copyright &copy; GrayMatterForge-
...
*/
class BsHtml extends CHtml // required in order to access the protected methods in CHtml
{
	//
	// TYPOGRAPHY
	// --------------------------------------------------
	...
	*/
	...
	{
		$color = GmfArray::popValue('color', $htmlOptions);
		if (GmfArray::popValue('muted', $htmlOptions, false)) {
	...
	*/
	...
	{
		if (GmfArray::popValue('small', $htmlOptions, false)) {
	...
	*/
	...
	{
		$paragraphOptions = GmfArray::popValue('paragraphOptions', $htmlOptions, array());
		$source = GmfArray::popValue('source', $htmlOptions);
		$sourceOptions = GmfArray::popValue('sourceOptions', $htmlOptions, array());
		$cite = GmfArray::popValue('cite', $htmlOptions);
		$citeOptions = GmfArray::popValue('citeOptions', $htmlOptions, array());
	...
	*/
	...
	{
		$type = GmfArray::popValue('type', $htmlOptions, self::HELP_TYPE_INLINE);
	...
	*/
	public static function bsForm(
		$layout = self::FORM_LAYOUT_VERTICAL,
		$action = '',
		$method = 'post',
		$htmlOptions = array()
	) {
		return self::beginBsForm($layout, $action, $method, $htmlOptions);
	}
	...
	* @return string the generated tag.
	*/
	public static function beginBsForm(
		$layout = self::FORM_LAYOUT_VERTICAL,
		$action = '',
		$method = 'post',
		$htmlOptions = array()
	...
	*/
	public static function statefulBsForm(
		$layout = self::FORM_LAYOUT_VERTICAL,
		$action = '',
		$method = 'post',
		$htmlOptions = array()
	) {
		return self::bsForm($layout, $action, $method, $htmlOptions)
	...
	*/
	...
	{
		$label = GmfArray::popValue('label', $htmlOptions, false);
		$labelOptions = GmfArray::popValue('labelOptions', $htmlOptions, array());
	...
	*/
	...
	{
		$displaySize = GmfArray::popValue('displaySize', $htmlOptions);
		$htmlOptions = self::normalizeInputOptions($htmlOptions);
	...
	*/
	public static function listBox($name, $select, $data, $htmlOptions = array())
	{
		...
		GmfArray::defaultValue('displaySize', 4, $htmlOptions);
	...
	*/
	...
	{
		$inline = GmfArray::popValue('inline', $htmlOptions, false);
		$separator = GmfArray::popValue('separator', $htmlOptions, ' ');
		$container = GmfArray::popValue('container', $htmlOptions, 'span');
		$containerOptions = GmfArray::popValue('containerOptions', $htmlOptions, array());
		$labelOptions = GmfArray::popValue('labelOptions', $htmlOptions, array());

		$items = array();
		$baseID = $containerOptions['id'] = GmfArray::popValue('baseID', $htmlOptions, parent::getIdByName($name));
	...
	*/
	public static function checkBoxList($name, $select, $data, $htmlOptions = array())
	{
		...
		$checkAll = GmfArray::popValue('checkAll', $htmlOptions);
		$checkAllLast = GmfArray::popValue('checkAllLast', $htmlOptions);
		if ($checkAll !== null) {
...
EOD;
	...
	*/
	...
	{
		$color = GmfArray::popValue('color', $htmlOptions);
		$groupOptions = GmfArray::popValue('groupOptions', $htmlOptions, array());
		$controlOptions = GmfArray::popValue('controlOptions', $htmlOptions, array());
		$label = GmfArray::popValue('label', $htmlOptions);
		$labelOptions = GmfArray::popValue('labelOptions', $htmlOptions, array());
		...
		$help = GmfArray::popValue('help', $htmlOptions, '');
		$helpOptions = GmfArray::popValue('helpOptions', $htmlOptions, array());
		if (!empty($help)) {
	...
	*/
	protected static function textInputField($type, $name, $value, $htmlOptions)
	{
		...
		$addOnOptions = GmfArray::popValue('addOnOptions', $htmlOptions, array());
		self::addCssClass($addOnClasses, $addOnOptions);

		$prepend = GmfArray::popValue('prepend', $htmlOptions, '');
		$prependOptions = GmfArray::popValue('prependOptions', $htmlOptions, array());
		if (!empty($prepend)) {
			$prepend = self::inputAddOn($prepend, $prependOptions);
		}

		$append = GmfArray::popValue('append', $htmlOptions, '');
		$appendOptions = GmfArray::popValue('appendOptions', $htmlOptions, array());
		if (!empty($append)) {
	...
	*/
	public static function activeRadioButtonList($model, $attribute, $data, $htmlOptions = array())
	{
		...
		$name = GmfArray::popValue('name', $htmlOptions);
		$uncheckValue = isset($htmlOptions['uncheckValue']) ? GmfArray::popValue('uncheckValue', $htmlOptions) : '';
	...
	*/
	public static function activeUneditableField($model, $attribute, $htmlOptions = array())
	{
		...
		GmfArray::removeValues(array('name', 'id'), $htmlOptions);
		return self::uneditableField($value, $htmlOptions);
	}
	...
	*/
	public static function activeControlGroup($type, $model, $attribute, $htmlOptions = array(), $data = array())
	{
		...
		$error = GmfArray::popValue('error', $htmlOptions, '');
	...
	*/
	...
	{
		$classes = array();
		if (GmfArray::getValue('append', $htmlOptions)) {
			$classes[] = 'input-append';
		}
		if (GmfArray::getValue('prepend', $htmlOptions)) {
	...
	*/
	protected static function normalizeInputOptions($options)
	{
		...
		$size = GmfArray::popValue('size', $options);
		if (GmfArray::popValue('block', $options, false)) {
	...
	*/
	public static function controls($controls, $htmlOptions = array())
	{
		...
		if (GmfArray::popValue('row', $htmlOptions, false)) {
			self::addCssClass('controls-row', $htmlOptions);
		}
		$before = GmfArray::popValue('before', $htmlOptions, '');
		$after = GmfArray::popValue('after', $htmlOptions, '');
	...
	*/
	public static function searchForm($action, $method = 'post', $htmlOptions = array())
	{
		...
		$inputOptions = GmfArray::popValue('inputOptions', $htmlOptions, array());
		$inputOptions = GmfArray::merge(array('type' => 'text', 'placeholder' => 'Search'), $inputOptions);
		$name = GmfArray::popValue('name', $inputOptions, 'search');
		$value = GmfArray::popValue('value', $inputOptions, '');
		$output = self::beginbsForm(self::FORM_LAYOUT_SEARCH, $action, $method, $htmlOptions);
	...
	*/
	public static function btn($type, $label, $htmlOptions = array())
	{
		...
		$size = GmfArray::popValue('size', $htmlOptions);
		if (!empty($size)) {
			self::addCssClass('btn-' . $size, $htmlOptions);
		}
		if (GmfArray::popValue('block', $htmlOptions, false)) {
			self::addCssClass('btn-block', $htmlOptions);
		}
		if (GmfArray::getValue('disabled', $htmlOptions, false)) {
			self::addCssClass('disabled', $htmlOptions);
			$htmlOptions['disabled'] = 'disabled';
		}
		$loading = GmfArray::popValue('loading', $htmlOptions);
		if (!empty($loading)) {
			$htmlOptions['data-loading-text'] = $loading;
		}
		if (GmfArray::popValue('toggle', $htmlOptions, false)) {
			$htmlOptions['data-toggle'] = 'button';
		}
		$icon = GmfArray::popValue('icon', $htmlOptions);
		$iconOptions = GmfArray::popValue('iconOptions', $htmlOptions, array());
		if (strpos($type, 'input') === false) {
			if (!empty($icon)) {
				$label = self::icon($icon, $iconOptions) . ' ' . $label;
			}
			$items = GmfArray::popValue('items', $htmlOptions);
		}
		$dropdownOptions = $htmlOptions;
		GmfArray::removeValues(array('groupOptions', 'menuOptions', 'dropup'), $htmlOptions);
	...
	*/
	...
	{
		$menuOptions = GmfArray::popValue('menuOptions', $htmlOptions, array());
		$groupOptions = GmfArray::popValue('groupOptions', $htmlOptions, array());
		self::addCssClass('btn-group', $groupOptions);
		if (GmfArray::popValue('dropup', $htmlOptions, false)) {
			self::addCssClass('dropup', $groupOptions);
		}
		$output = self::openTag('div', $groupOptions);
		if (GmfArray::popValue('split', $htmlOptions, false)) {
	...
	*/
	...
	{
		$url = GmfArray::popValue('url', $htmlOptions, '#');
		$ajaxOptions = GmfArray::popValue('ajaxOptions', $htmlOptions, array());
		switch ($type) {
	...
	*/
	...
	{
		$type = GmfArray::popValue('type', $htmlOptions);
		if (!empty($type)) {
	...
	*/
	...
	{
		GmfArray::defaultValue('role', 'menu', $htmlOptions);
	...
	*/
	public static function buttonGroup(array $buttons, $htmlOptions = array())
	{
		if (!empty($buttons)) {
			...
			if (GmfArray::popValue('vertical', $htmlOptions, false)) {
				self::addCssClass('btn-group-vertical', $htmlOptions);
			}
			$toggle = GmfArray::popValue('toggle', $htmlOptions);
			if (!empty($toggle)) {
			...
			$parentOptions = array(
				'color' => GmfArray::popValue('color', $htmlOptions),
				'size' => GmfArray::popValue('size', $htmlOptions),
				'disabled' => GmfArray::popValue('disabled', $htmlOptions)
			);
			...
			foreach ($buttons as $buttonOptions) {
				...
				$options = GmfArray::popValue('htmlOptions', $buttonOptions, array());
				if (!empty($options)) {
					$buttonOptions = GmfArray::merge($options, $buttonOptions);
				}
				$buttonLabel = GmfArray::popValue('label', $buttonOptions, '');
				$buttonOptions = GmfArray::copyValues(array('color', 'size', 'disabled'), $parentOptions, $buttonOptions);
				$items = GmfArray::popValue('items', $buttonOptions, array());
				if (!empty($items)) {
					$output .= self::buttonDropdown($buttonLabel, $items, $buttonOptions);
				} else {
					$ajaxOptions = GmfArray::popValue('ajaxOptions', $buttonOptions, array());
					if(!empty($ajaxOptions)) {
						$output .= self::ajaxButton($buttonLabel, GmfArray::popValue('url', $ajaxOptions, '#'), $ajaxOptions, $buttonOptions);
					} else {
	...
	public static function buttonToolbar(array $groups, $htmlOptions = array())
	{
		if (!empty($groups)) {
			...
			foreach ($groups as $groupOptions) {
				...
				$items = GmfArray::popValue('items', $groupOptions, array());
				if (empty($items)) {
					continue;
				}
				// todo: consider removing the support for htmlOptions.
				$options = GmfArray::popValue('htmlOptions', $groupOptions, array());
				if (!empty($options)) {
					$groupOptions = GmfArray::merge($options, $groupOptions);
				}
				$groupOptions = GmfArray::copyValues(array('color', 'size', 'disabled'), $parentOptions, $groupOptions);
	...
	*/
	public static function buttonDropdown($label, $items, $htmlOptions = array())
	{
		...
		$type = GmfArray::popValue('type', $htmlOptions, self::BUTTON_TYPE_LINKBUTTON);
		return self::btn($type, $label, $htmlOptions);
	}
	...
	public static function navList($items, $htmlOptions = array())
	{
		foreach ($items as $i => $itemOptions) {
			...
			if (!isset($itemOptions['url']) && !isset($itemOptions['items'])) {
				$label = GmfArray::popValue('label', $itemOptions, '');
	...
	*/
	public static function nav($type, $items, $htmlOptions = array())
	{
		...
		$stacked = GmfArray::popValue('stacked', $htmlOptions, false);
		...
		if (!empty($items)) {
			...
			foreach ($items as $itemOptions) {
				if (is_string($itemOptions)) {
					...
					// todo: consider removing the support for htmlOptions.
					$options = GmfArray::popValue('htmlOptions', $itemOptions, array());
					if (!empty($options)) {
						$itemOptions = GmfArray::merge($options, $itemOptions);
					}
					$label = GmfArray::popValue('label', $itemOptions, '');
					if (GmfArray::popValue('active', $itemOptions, false)) {
						self::addCssClass('active', $itemOptions);
					}
					if (GmfArray::popValue('disabled', $itemOptions, false)) {
						self::addCssClass('disabled', $itemOptions);
					}
					if (!isset($itemOptions['linkOptions'])) {
						$itemOptions['linkOptions'] = array();
					}
					$icon = GmfArray::popValue('icon', $itemOptions);
					if (!empty($icon)) {
						$label = self::icon($icon) . ' ' . $label;
					}
					$items = GmfArray::popValue('items', $itemOptions, array());
					$url = GmfArray::popValue('url', $itemOptions, false);
					if (empty($items)) {
						if (!$url) {
	...
	*/
	...
	{
		GmfArray::defaultValue('role', 'menuitem', $htmlOptions);
		$linkOptions = GmfArray::popValue('linkOptions', $htmlOptions, array());
	...
	*/
	protected static function menuDropdown($label, $url, $items, $htmlOptions, $depth = 0)
	{
		...
		if ($depth === 0) {
			...
			GmfArray::defaultValue('id', $defaultId, $menuOptions);
	...
	*/
	public static function tabbable($type, $tabs, $htmlOptions = array())
	{
		...
		$placement = GmfArray::popValue('placement', $htmlOptions);
		if (!empty($placement)) {
			self::addCssClass('tabs-' . $placement, $htmlOptions);
		}
		$menuOptions = GmfArray::popValue('menuOptions', $htmlOptions, array());
		$contentOptions = GmfArray::popValue('contentOptions', $htmlOptions, array());
	...
	*/
	protected static function normalizeTabs($tabs, &$panes, &$i = 0)
	{
		...
		foreach ($tabs as $tabOptions) {
			...
			$menuItem = array();
			$menuItem['icon'] = GmfArray::popValue('icon', $tabOptions);
			$menuItem['label'] = GmfArray::popValue('label', $tabOptions, '');
			$menuItem['active'] = GmfArray::getValue('active', $tabOptions, false);
			$menuItem['disabled'] = GmfArray::popValue('disabled', $tabOptions, false);
			$menuItem['linkOptions'] = GmfArray::popValue('linkOptions', $tabOptions, array());
			$menuItem['htmlOptions'] = GmfArray::popValue('htmlOptions', $tabOptions, array());
			$items = GmfArray::popValue('items', $tabOptions, array());
			if (!empty($items)) {
				$menuItem['linkOptions']['data-toggle'] = 'dropdown';
				$menuItem['items'] = self::normalizeTabs($items, $panes, $i);
			} else {
				$paneOptions = GmfArray::popValue('paneOptions', $tabOptions, array());
				$id = $paneOptions['id'] = GmfArray::popValue('id', $tabOptions, 'tab_' . ++$i);
				$menuItem['linkOptions']['data-toggle'] = 'tab';
				$menuItem['url'] = '#' . $id;
				self::addCssClass('tab-pane', $paneOptions);
				if (GmfArray::popValue('fade', $tabOptions, true)) {
					self::addCssClass('fade', $paneOptions);
				}
				if (GmfArray::popValue('active', $tabOptions, false)) {
					self::addCssClass('active in', $paneOptions);
				}
				$paneContent = GmfArray::popValue('content', $tabOptions, '');
	...
	*/
	public static function navbar($content, $htmlOptions = array())
	{
		...
		$display = GmfArray::popValue('display', $htmlOptions);
		if (!empty($display)) {
		...
		$innerOptions = GmfArray::popValue('innerOptions', $htmlOptions, array());
		$tag = GmfArray::popValue('tag', $htmlOptions, 'div');
	...
	*/
	...
	{
		$divider = GmfArray::popValue('divider', $htmlOptions, '/');
		self::addCssClass('breadcrumb', $htmlOptions);
		$output = self::openTag('ul', $htmlOptions);
		foreach ($links as $label => $url) {
			if ((is_string($label) && strlen($label)>0) || is_array($url)) {
	...
	*/
	public static function pagination(array $items, $htmlOptions = array())
	{
		if (!empty($items)) {
			...
			$align = GmfArray::popValue('align', $htmlOptions);
			if (!empty($align)) {
				self::addCssClass('pagination-' . $align, $htmlOptions);
			}
			$listOptions = GmfArray::popValue('listOptions', $htmlOptions, array());
			$output = self::openTag('div', $htmlOptions);
	...
	*/
	...
	{
		$linkOptions = GmfArray::popValue('linkOptions', $htmlOptions, array());
		if (GmfArray::popValue('active', $htmlOptions, false)) {
			self::addCssClass('active', $htmlOptions);
		}
		if (GmfArray::popValue('disabled', $htmlOptions, false)) {
			self::addCssClass('disabled', $htmlOptions);
		}
		$content = self::link($label, $url, $linkOptions);
		return self::tag('li', $htmlOptions, $content);
	...
	*/
	...
	{
		$linkOptions = GmfArray::popValue('linkOptions', $htmlOptions, array());
		if (GmfArray::popValue('previous', $htmlOptions, false)) {
			self::addCssClass('previous', $htmlOptions);
		}
		if (GmfArray::popValue('next', $htmlOptions, false)) {
	...
	*/
	public static function heroUnit($heading, $content, $htmlOptions = array())
	{
		...
		$headingOptions = GmfArray::popValue('headingOptions', $htmlOptions, array());
		$output = self::openTag('div', $htmlOptions);
	...
	*/
	public static function pageHeader($heading, $subtext, $htmlOptions = array())
	{
		...
		$headerOptions = GmfArray::popValue('headerOptions', $htmlOptions, array());
		$subtextOptions = GmfArray::popValue('subtextOptions', $htmlOptions, array());
		$output = self::openTag('div', $htmlOptions);
	...
	*/
	public static function thumbnails(array $thumbnails, $htmlOptions = array())
	{
		if (!empty($thumbnails)) {
			...
			$defaultSpan = GmfArray::popValue('span', $htmlOptions, 3);
			$output = self::openTag('ul', $htmlOptions);
			foreach ($thumbnails as $thumbnailOptions) {
				...
				// todo: consider removing the support for htmlOptions.
				$options = GmfArray::popValue('htmlOptions', $thumbnailOptions, array());
				if (!empty($options)) {
					$thumbnailOptions = GmfArray::merge($options, $thumbnailOptions);
				}
				$thumbnailOptions['itemOptions']['span'] = GmfArray::popValue('span', $thumbnailOptions, $defaultSpan);
				$caption = GmfArray::popValue('caption', $thumbnailOptions, '');
				$captionOptions = GmfArray::popValue('captionOptions', $thumbnailOptions, array());
				self::addCssClass('caption', $captionOptions);
				$label = GmfArray::popValue('label', $thumbnailOptions);
				$labelOptions = GmfArray::popValue('labelOptions', $thumbnailOptions, array());
				if (!empty($label)) {
					$caption = self::tag('h3', $labelOptions, $label) . $caption;
				}
				$content = !empty($caption) ? self::tag('div', $captionOptions, $caption) : '';
				$image = GmfArray::popValue('image', $thumbnailOptions);
				$imageOptions = GmfArray::popValue('imageOptions', $thumbnailOptions, array());
				$imageAlt = GmfArray::popValue('alt', $imageOptions, '');
				if (!empty($image)) {
					$content = parent::image($image, $imageAlt, $imageOptions) . $content;
				}
				$url = GmfArray::popValue('url', $thumbnailOptions, false);
				$output .= $url !== false
	...
	*/
	...
	{
		$itemOptions = GmfArray::popValue('itemOptions', $htmlOptions, array());
	...
	*/
	public static function alert($color, $message, $htmlOptions = array())
	{
		...
		if (GmfArray::popValue('in', $htmlOptions, true)) {
			self::addCssClass('in', $htmlOptions);
		}
		if (GmfArray::popValue('block', $htmlOptions, false)) {
			self::addCssClass('alert-block', $htmlOptions);
		}
		if (GmfArray::popValue('fade', $htmlOptions, true)) {
			self::addCssClass('fade', $htmlOptions);
		}
		$closeText = GmfArray::popValue('closeText', $htmlOptions, self::CLOSE_TEXT);
		$closeOptions = GmfArray::popValue('closeOptions', $htmlOptions, array());
	...
	*/
	public static function progressBar($width = 0, $htmlOptions = array())
	{
		...
		if (GmfArray::popValue('striped', $htmlOptions, false)) {
			self::addCssClass('progress-striped', $htmlOptions);
		}
		if (GmfArray::popValue('animated', $htmlOptions, false)) {
			self::addCssClass('active', $htmlOptions);
		}
		$barOptions = GmfArray::popValue('barOptions', $htmlOptions, array());
		$content = GmfArray::popValue('content', $htmlOptions);
		if (!empty($content)) {
	...
	public static function stackedProgressBar(array $bars, $htmlOptions = array())
	{
		if (!empty($bars)) {
			...
			foreach ($bars as $barOptions) {
				...
				$width = GmfArray::popValue('width', $barOptions, 0);
				$tmp = $totalWidth;
				$totalWidth += $width;
				if ($totalWidth > 100) {
					$width = 100 - $tmp;
	...
	public static function medias(array $items, $tag = 'div')
	{
		if (!empty($items)) {
			...
			foreach ($items as $itemOptions) {
				...
				$image = GmfArray::popValue('image', $itemOptions);
				$heading = GmfArray::popValue('heading', $itemOptions, '');
				$content = GmfArray::popValue('content', $itemOptions, '');
				GmfArray::defaultValue('tag', $tag, $itemOptions);
	...
	*/
	public static function media($image, $heading, $content, $htmlOptions = array())
	{
		...
		$linkOptions = GmfArray::popValue('linkOptions', $htmlOptions, array());
		GmfArray::defaultValue('pull', self::PULL_LEFT, $linkOptions);
		$imageOptions = GmfArray::popValue('imageOptions', $htmlOptions, array());
		...
		$output = self::openTag($tag, $htmlOptions);
		$alt = GmfArray::popValue('alt', $imageOptions, '');
		$href = GmfArray::popValue('href', $linkOptions, '#');
		if (!empty($image)) {
	...
	*/
	protected static function close($tag, $label, $htmlOptions = array())
	{
		...
		$dismiss = GmfArray::popValue('dismiss', $htmlOptions);
		if (!empty($dismiss)) {
	...
	*/
	public static function modalHeader($content, $htmlOptions = array())
	{
		...
		$headingOptions = GmfArray::popValue('headingOptions', $htmlOptions, array());
		$closeLabel = GmfArray::popValue('closeLabel', $htmlOptions, self::CLOSE_TEXT);
	...
	*/
	protected static function tooltipPopover($label, $url, $title, $htmlOptions)
	{
		...
		if (GmfArray::popValue('animation', $htmlOptions)) {
			$htmlOptions['data-animation'] = 'true';
		}
		if (GmfArray::popValue('html', $htmlOptions)) {
			$htmlOptions['data-html'] = 'true';
		}
		$selector = GmfArray::popValue('selector', $htmlOptions);
		if (!empty($selector)) {
		...
		$trigger = GmfArray::popValue('trigger', $htmlOptions);
		if (!empty($trigger)) {
			$htmlOptions['data-trigger'] = $trigger;
		}
		if (($delay = GmfArray::popValue('delay', $htmlOptions)) !== null) {
	...
	*/
	...
	{
		if (!empty($items)) {
			$id = GmfArray::getValue('id', $htmlOptions, parent::ID_PREFIX . parent::$count++);
			GmfArray::defaultValue('id', $id, $htmlOptions);
			$selector = '#' . $id;
			self::addCssClass('carousel', $htmlOptions);
			if (GmfArray::popValue('slide', $htmlOptions, true)) {
				self::addCssClass('slide', $htmlOptions);
			}
			$interval = GmfArray::popValue('data-interval', $htmlOptions);
			if ($interval) {
				$htmlOptions['data-interval'] = $interval;
			}
			$pause = GmfArray::popValue('data-pause', $htmlOptions);
			if ($pause) {
				$htmlOptions['data-pause'] = $pause;
			}
			$indicatorOptions = GmfArray::popValue('indicatorOptions', $htmlOptions, array());
			$innerOptions = GmfArray::popValue('innerOptions', $htmlOptions, array());
			self::addCssClass('carousel-inner', $innerOptions);
			$prevOptions = GmfArray::popValue('prevOptions', $htmlOptions, array());
			$prevLabel = GmfArray::popValue('label', $prevOptions, '&lsaquo;');
			$nextOptions = GmfArray::popValue('nextOptions', $htmlOptions, array());
			$nextLabel = GmfArray::popValue('label', $nextOptions, '&rsaquo;');
			$hidePrevAndNext = GmfArray::popValue('hidePrevAndNext', $htmlOptions, false);
			$output = self::openTag('div', $htmlOptions);
			...
			foreach ($items as $i => $itemOptions) {
				...
				$content = GmfArray::popValue('content', $itemOptions, '');
				$image = GmfArray::popValue('image', $itemOptions, '');
				$imageOptions = GmfArray::popValue('imageOptions', $itemOptions, array());
				$imageAlt = GmfArray::popValue('alt', $imageOptions, '');
				if (!empty($image)) {
					$content = parent::image($image, $imageAlt, $imageOptions);
				}
				$label = GmfArray::popValue('label', $itemOptions);
				$caption = GmfArray::popValue('caption', $itemOptions);
	...
	*/
	public static function carouselItem($content, $label, $caption, $htmlOptions = array())
	{
		...
		$overlayOptions = GmfArray::popValue('overlayOptions', $htmlOptions, array());
		self::addCssClass('carousel-caption', $overlayOptions);
		$labelOptions = GmfArray::popValue('labelOptions', $htmlOptions, array());
		$captionOptions = GmfArray::popValue('captionOptions', $htmlOptions, array());
		$url = GmfArray::popValue('url', $htmlOptions, false);
		if ($url !== false) {
	...
	*/
	...
	{
		$span = GmfArray::popValue('span', $htmlOptions);
		if (!empty($span)) {
	...
	*/
	...
	{
		$pull = GmfArray::popValue('pull', $htmlOptions);
		if (!empty($pull)) {
	...
	*/
	...
	{
		$align = GmfArray::popValue('textAlign', $htmlOptions);
		if (!empty($align)) {
